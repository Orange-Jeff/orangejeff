...
// Functionality: Audio segmentation for speaker voice separation
?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function handleFile(file) {
                ...
                // Rest of the existing handleFile code...
            ...
            // Add a function to control disabled state of UI elements
            function setControlsEnabled(enabled) {
                // Control buttons
                document.querySelectorAll('#playPause, #jumpStart, #jumpEnd, #jumpBack, #jumpForward, #zoomIn, #zoomOut, #zoomFit, #clearRegions').forEach(btn => {
                    btn.disabled = !enabled;
                });

                // Region creation buttons
                document.querySelectorAll('#speaker1Region, #speaker2Region, #trashRegion, #undoRegion').forEach(btn => {
                    btn.disabled = !enabled;
                });

                // Process button
                processAudioBtn.disabled = !enabled;
            }

            // Disable controls initially
            setControlsEnabled(false);

            // Update handleFile function to enable controls
            function handleFile(file) {
                if (!file || !(file.name.toLowerCase().endsWith('.wav'))) {
                    updateStatus('Please select a valid WAV file', 'error');
                    return;
            ...
            // When page loads, update the waveform to show empty placeholder state
            wavesurfer.on('ready', () => {
                // Existing ready code...
            });

            // Update the event handlers for buttons to check enabled state
            document.querySelectorAll('#speaker1Region, #speaker2Region, #trashRegion').forEach(btn => {
            ...
            // Initial status message
            updateStatus('Ready to load audio file. Please open a WAV file to begin.', 'info');

            // Add CSS to show disabled state clearly
            const style = document.createElement('style');
            style.textContent = `
                .button-blue:disabled, .command-button:disabled {
                    opacity: 0.5;
                    cursor: not-allowed;
                    background-color: #6c757d;
                }
